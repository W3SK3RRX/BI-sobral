...
const EditarDashboard = () => {
  ...
  useEffect(() => {
    ...
    dashboardAPI.getDashboardById(id).then((data) => {
      setNome(data.nome);
      setDescricao(data.descricao);
      setLink(data.link);
      setCategoria(data.categoria);
      setNivelMinimo(data.nivel_minimo);
      setUsuariosPermitidos(data.usuarios_permitidos || []);
    }).catch(console.error);
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    ...
    await dashboardAPI.updateDashboard(id, dadosAtualizados);
    navigate('/admin');
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-orange-light">
      ...
      >
        <h2 className="text-2xl font-bold mb-6">Editar Dashboard</h2>
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow-md space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={e => setUsuariosPermitidos(Array.from(e.target.selectedOptions, option => option.value))}
...
export default EditarDashboard;